...
static int te_relocate(uintptr_t new_addr, void *te)
{
	...
	/* Further adjust address to account for addressing when using secure boot */
	#if defined (_SECUREBOOT_)
	adj -= 0x10000;
	#endif /* _SECUREBOOT_ */
...
ssize_t fsp_component_relocate(uintptr_t new_addr, void *fsp, size_t size)
{
	...
	while (offset < size) {
		...
		if (offset == 0) {
		    /*
		     * Fixes an address at the beginning of FSP to account
		     * for addressing changes in secure boot
		     */
		    #if defined(_SECUREBOOT_)
