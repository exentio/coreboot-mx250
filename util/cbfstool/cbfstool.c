...
static int cbfs_add_master_header(void)
{
	...
	/*
	 * Fixes an offset at 0x40 from CBFS master header
	 * (0xC00140 for 16M image)
	 */
	#if defined(_SECUREBOOT_)
	h->romsize += htonl(0x10000);
	#endif /* _SECUREBOOT_ */
	...
	/* Adjusts pointer at reset vector to account for shifted addressing */
